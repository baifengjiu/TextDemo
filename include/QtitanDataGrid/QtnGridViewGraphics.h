...
class GridViewLayoutPrimitive : public QObject, public ScrollableGraphicPrimitive
{
    Q_OBJECT
public:
...
class GridColumnEditorPrimitive : public EmbedGraphicPrimitive
{
public:
    friend class GridColumnPrimitive;
    GridColumnEditorPrimitive(GridColumnPrimitive* parent, GridColumnBase* column);
    virtual ~GridColumnEditorPrimitive();
protected:
...
inline bool qtn_is_row_selection_policy(GridViewBase* view)
{
    return (view->optionsref()->selectionPolicy() == SelectSingleRow || view->optionsref()->selectionPolicy() == SelectMultiRow);
}
