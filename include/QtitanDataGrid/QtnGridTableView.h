...
public:
    ...
    friend class GridTableQuickRowButtonPrimitive;
...
public:
    explicit GridTableView(Grid* grid);
...
protected:
    ...
    virtual void bestFitForView(BestFitMode mode);
    GridTableViewPrimitive* layout() const;
    virtual bool printHeader(QPainter& painter, int& pageLength, int& itemIndex);
private:
    void collectColumsForSelect(const QRect& rubberRect, QList<GridDataTableColumn *>& columns);
...
public:
    ...
    virtual GridTableColumnBase* getLastColumnForSource();
protected:
    virtual void beginEvent();
    virtual void moveEvent(const QPoint& pos);
    virtual void acceptEvent();
...
class GridCellDragObject: public GridNativeDragObject
{
    Q_OBJECT
public:
    GridCellDragObject(GridViewBase* view, GridTableCellPrimitive* gp, const QPoint& cellPos);
    ...
    bool checkCell(ColumnMovePosition position, GridTableCellPrimitive* gp);
...
protected:
    ...
    virtual void endEvent();
private:
    QPixmap selectedCellsToPixmap();
    QPixmap selectedRowsToPixmap();
    QList<int> removeDraggedContent();
    GridViewBase* m_view;
    bool m_initialized;
    QPoint m_pos;
    ColumnArrowWidget* m_upArrow;
...
public:
    ...
    GridTableView* view() const;
protected:
    virtual void columnCreated(const GridDataTableColumn* column);
    virtual void columnDestroying(const GridDataTableColumn* column);
...
public:
    ...
    GridRowLayoutItem* getVirtualRowByPos(const QPoint& pos);
private: 
...
{
public:
    GridTableColumnEditor(int borderThickness = 1);
