...
class ScrollableGraphicPrimitive;
class EmbedGraphicPrimitive;
...
public:
    ...
    friend class GridNativeDragObject;
...
protected:
    ...
    void widgetFocusIn(QFocusEvent* e);
    void widgetFocusOut(QFocusEvent* e);
...
private:
    ...
    QPoint m_pressPos;
#endif
    QSet<EmbedGraphicPrimitive *> m_embedList;
private Q_SLOTS:
...
protected:
    ...
    void updateCursor(bool overrideCursor = true);
private:
...
class QTITAN_EXPORT GraphicPrimitive
{
public:
    struct FixedState
    {
...
protected:
    ...
    virtual void wheelEvent(const QPoint& pos, QWheelEvent* e);
    /* focus events */
    virtual void focusInEvent(QFocusEvent* e);
    virtual void focusOutEvent(QFocusEvent* e);
    /* mouse events */
    virtual void mouseMoveEvent(const QPoint& pos, QMouseEvent* e);
    virtual void mousePressEvent(const QPoint& pos, QMouseEvent* e);
    virtual void mouseReleaseEvent(const QPoint& pos, QMouseEvent* e);
    virtual void mouseDoubleClickEvent(const QPoint& pos, QMouseEvent* e);
    virtual void mouseOverEvent(const QPoint& pos);
    virtual void mouseOutEvent();
    /* keyboard events */
    virtual void keyPressEvent(QKeyEvent* e);
    virtual void keyReleaseEvent(QKeyEvent* e);
    virtual void shortcutOverrideEvent(QKeyEvent* e);
    /* block events */
    virtual void windowBlockedEvent(QEvent* e);
    virtual void windowUnBlockedEvent(QEvent* e);
    ...
    virtual void showContextMenuEvent(const QPoint& pos, QContextMenuEvent* e);
...
public:
    ...
    void updateWidgetGeometry(bool force = false);
protected:
    virtual void paintEvent(QPainter& painter);
    virtual void resizeEvent(const QRect& rect);
    /* mouse events */
...
public:
    friend class ScrollableGraphicPrimitive;
    GraphicPrimitiveScrollBar(Qt::Orientation, ScrollableGraphicPrimitive* primitive);
protected:
    virtual void mouseMoveEvent(QMouseEvent *);
    virtual void mousePressEvent(QMouseEvent *);
private:
    ScrollableGraphicPrimitive* m_primitive;
protected Q_SLOTS:
...
extern void draw_focus_frame(QPainter& painter, const QColor& color, const QRect& rect, QWidget* widget);
...
public:
    EffectAbstract(GraphicPrimitive* gp);
    virtual ~EffectAbstract();
    int duration() const;
...
class SmoothEffect: public EffectAbstract
{
public:
    SmoothEffect(GraphicPrimitive* gp);
    void setInitialState(const QImage& iState);
    void setInitialStateToEmpty(int width = 0, int height = 0);
...
class TimeoutEffect: public EffectAbstract
{
public:
    TimeoutEffect(GraphicPrimitive* gp);
    virtual void execute();
};

typedef QHash<GraphicPrimitive *, EffectAbstract *> EffectHash;

class Effects: public QObject
{
public:
    friend class SmoothEffect;
    Effects(QWidget* control);
    virtual ~Effects();
    void run(SmoothEffect* effect);
    void stop(SmoothEffect* effect);
    void run(TimeoutEffect* effect);
    void stop(TimeoutEffect* effect);
    void timeout(GraphicPrimitive* gp, int delay);
    SmoothEffect* findEffect(GraphicPrimitive* gp);
    void stopAnyEffects(GraphicPrimitive* gp);
protected:
    virtual bool event(QEvent* e);
    void update(bool smooth);
private:
    QWidget* m_control;
    EffectHash m_smoothHash;
    EffectHash m_timeoutHash;
    QBasicTimer m_timer;
    QRegion m_dirty;
};
