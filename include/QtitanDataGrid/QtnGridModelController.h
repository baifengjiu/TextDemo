...
public:
    ...
    virtual int getFirstColumnIndex() const = 0;
    virtual void modelAboutToBeUpdated(bool clearAll) = 0;
    virtual void modelUpdated() = 0;
    virtual void columnInserted(int modelColumn) = 0;
    virtual void columnRemoved(GridModelDataBinding* dataBinding) = 0;
...
public:
    explicit GridModelDataBinding(GridModelController* modelController, int column);
    ...
    const DefaultValueMap& defaultValues() const;
...
public:
    ...
    Qt::ItemFlags getFlags(int modelRowIndex) const;
    QModelIndex findRelationModelIndex(const QVariant& foreignKey, Qt::ItemDataRole role, Qt::ItemDataRole& relationRole) const;
private:
...
public:
...
#endif
    GridModelDataBinding* getDataBinding(int modelColumn) const;
    ...
    bool removeModelRows(int modelRow, int rowCount = 1);
    ...
    QMimeData* createMimeData(const QModelIndexList& indexes);
public:
    GridModelDataBinding* previewRowDataBinding() const;
    const GridSortColumnRules& sortRules() const;
    void setSortRules(const GridSortColumnRules& sortRules);
public:
    void clearSelect();
    ...
    bool submitData();
    const GridControllerChanges& changes() const;
Q_SIGNALS:
...
protected:
    void insertDataBindings(int start, int count);
    void destroyDataBindings();
    void destroyDataBinding(GridModelDataBinding* dataBinding);
private:
    bool setFocusInternal(GridRow* row, int columnIndex);
    void ensureFocus();
...
protected Q_SLOTS:
    ...
    void modelDestroyed();
private:
    int findNearestRecordByModelRow(int modelRowIndex);
...
#endif
    QVector<GridModelDataBinding *> m_dataBindings;
    QHash<QString, GridModelDataBinding *> m_bindingsResetCache;
...
public:
    ...
    void clearForDataBinding(GridModelDataBinding* dataBinding);
    void clear();
...
int qtn_variant_compare(const QVariant &v1, const QVariant& v2, bool caseSensitivity = false, bool localeAware = false);
void qtn_selection_range_compact(SelectionRanges& target);
